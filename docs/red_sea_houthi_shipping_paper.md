...
本文的方法设计与三条经验假设一一对应。H1 关注安全事件本身是否具有时间持续性，因此在 VAR 的 `attack_count` 方程中观察袭击事件滞后项是否显著。H2 关注安全事件对航道结构的动态影响，因此使用 VAR 和脉冲响应函数观察曼德海峡与好望角通行量在冲击后的多周变化。H3 关注 2023 年 11 月 19 日断点前后航道结构是否发生突变，因此使用事件研究、中断时间序列和 Chow 检验。
...
其中 `post_t` 表示日期是否在 2023 年 11 月 19 日及之后，`time_after_t` 表示断点后的时间趋势。`beta_2` 衡量断点后的即时水平变化，`beta_3` 衡量断点后的斜率变化。由于日度航运数据可能存在自相关，本文使用 Newey-West / HAC 标准误，并设置最大滞后为 14 天。
