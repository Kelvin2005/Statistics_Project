...
\title{胡塞武装相关安全事件对红海关键航道通行量与绕航行为的统计分析}
...
自 2023 年末以来，红海及亚丁湾安全局势恶化，对苏伊士运河、曼德海峡和好望角航线产生了持续影响。本文基于
ACLED 事件数据和 IMF PortWatch / PortStraitWatch 航道通行指标，研究胡塞武装相关安全事件与红海关键航道通行量、船舶绕航行为之间的统计关系。
研究将 ACLED 中胡塞、红海、亚丁湾相关事件按日期聚合，并保留爆炸/远程暴力、战斗、针对平民暴力和战略发展等更接近安全冲击的事件类型；
同时使用 PortWatch 中苏伊士运河、曼德海峡和好望角的日度通行数据，构造通行量、绕航指数和苏伊士通行占比。方法上，本文首先使用向量自回归模型和脉冲响应函数考察袭击事件、曼德海峡通行量和好望角通行量之间的动态关系；其次围绕
2023 年 11 月 19 日 Galaxy Leader 劫持事件构造事件研究、中断时间序列和 Chow 结构突变检验，并使用 Newey-West
稳健标准误修正日度时间序列中的异方差和自相关问题。结果显示，安全事件与航道通行结构之间存在一定动态关联，特别是断点后好望角通行量、好望角/苏伊士绕航指数显著上升，苏伊士和曼德海峡通行量显著下降。
本文据此将结论限定在聚合航道层面的准实验式统计关联，不讨论当前数据无法验证的单船风险、AIS 开关影响或船型敏感性问题。本文的贡献在于将冲突事件数据与航运通行数据结合，为红海危机下航运路线调整提供一个可复现的统计分析框架。
...
报道也在同日确认该船被胡塞武装扣押 \citep{marad2023redsea,pbs2023houthi}。本文因此把 2023 年 11 月 19
...
\citep{unctad2024troubled,imf2024redsea}。

在这一背景下，一个自然的统计问题是：
胡塞武装相关安全事件是否改变了红海关键航道的通行量，并推动船舶绕行好望角？
如果存在影响，这种影响是立即发生，还是具有滞后性？
...
\item
  2023 年 11 月 19 日红海危机升级节点后，苏伊士、曼德海峡、好望角和绕航指数是否出现结构性变化？
\end{enumerate}
...
\citep{unctad2024troubled,imf2024redsea,eia2024redsea,notteboom2024red}。
...
等 \citep{acled_codebook,acled_event_types}。对本文而言，Protests 和 Riots
...
chokepoints
的日度通行情况 \citep{imf_oxford_portwatch2023}。与传统贸易统计相比，这类高频通行指标更适合识别突发事件下航运活动的短期变化。
...
\citep{sims1980macroeconomics,lutkepohl2005new,bernal2017interrupted,newey1987simple}。
...
\[
C_{r,t}d=C^{fuel}_{r,t}+C^{time}_{r,t}+C^{ins}_{r,t}+C^{risk}_{r,t},
\]
...
的风险概率或风险感知强度，\(L_{r,t}\)
是一旦发生袭击、扣押、延误或改港时的预期损失。本文用 ACLED 聚合安全事件数作为 \(p_{r,t}\)的代理变量。
...
本文的数据能够验证``聚合航运通行结构是否随安全事件变化''。三类研究问题与数据验证能力的关系如下：
...
因果识别部分采用``外生断点 + 高频结果变量''的准实验思路。2023
...
若红海危机升级确实影响航运路线选择，应当看到一条完整的推断链：先是断点后红海相关通道的通行量下降，其次是好望角替代航线通行量上升，再次是绕航指数上升、苏伊士占比下降。事件研究用于描述这条曲线；中断时间序列用于检验断点后的水平和趋势变化；结构突变检验用于判断断点前后趋势关系是否整体改变。
...
过滤后共有 10,037 条事件。随后将事件按日期整合为日度袭击事件数
...
本文将``绕航指数''定义为好望角通行量与苏伊士运河通行量之比，用于衡量船舶从红海/苏伊士路径转向好望角路径的相对强度。记好望角日通行量为
\(Q^{Cape}_t\)，苏伊士运河日通行量为 \(Q^{Suez}_t\)，则绕航指数为：
...
当 \(RI_t\)
上升时，表示相对于苏伊士路径，更多船舶选择或经过好望角路径；当
\(RI_t\)
下降时，则表示苏伊士路径的相对通行重要性增强。为了从相反方向观察苏伊士路径的份额变化，本文同时构造苏伊士通行占比：
...
\texttt{rerouting\_index\_total} & 绕航指数，即好望角通行量与苏伊士通行量之比 \\
\texttt{suez\_share\_total} &
苏伊士在苏伊士和好望角合计通行量中的占比 \\
\end{longtable}

因此，\(RI_t\) 与 \(SS_t\)
分别从``替代航线扩张''和``原航线份额下降''两个角度对应问题二和问题三。
...
得到安全事件和主要航道通航船只的周度变化图：

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../../plot/acled_top_locations_weekly_events.png}}
\caption{重点地点安全事件周度数量变化}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../../plot/portwatch_chokepoints_weekly_ship_counts.png}}
...
\citep{sims1980macroeconomics,lutkepohl2005new}：
...
后再转换为相对基准期的百分比变化；对绕航指数和苏伊士占比，则计算相对基准期的水平差。
...
标准误，并设置最大滞后为 14 天 \citep{newey1987simple}。HAC 协方差矩阵可写为：
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../../plot/red_sea_event_study_2023_11_19.png}}
...
\centering
\includegraphics[width=\linewidth]{../../plot/red_sea_interrupted_time_series_2023_11_19.png}
\caption{中断时间序列拟合曲线}
\label{fig:red-sea-interrupted-time-series}
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\linewidth}
\centering
\includegraphics[width=\linewidth]{../../plot/red_sea_structural_break_2023_11_19.png}
\caption{结构突变诊断曲线}
\label{fig:red-sea-structural-break}
\end{minipage}
\end{figure}
